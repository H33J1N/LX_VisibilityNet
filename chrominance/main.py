"""
Visibility Enhancement Network: Chrominance
@Author: Heejin Lee
"""
...
parser.add_argument('--lr', dest='lr', type=float, default=0.00008, help='learning rate')
...
def visibility_train(visibility_enhance):
    ...
    # Enhanced Y image
    # ../dataset/train_Y/{condition}/

    train_y_data_names = glob('{}_Y/{}/*.png'.format(args.train_dir[:-1], args.condition))
    train_y_data_names.sort()
    assert len(train_data_names) == len(train_y_data_names)
    ...
    for idx in range(len(train_data_names)):
        ...
        train_y_im = load_images_luminance(train_y_data_names[idx], is_resize=True)

        train_im = np.concatenate([train_im, train_y_im], axis=-1)
        train_data.append(train_im)
        train_y_data.append(train_y_im)

    valid_data = []
    valid_y_data = []

    valid_data_name = glob('{}/*.*'.format(args.valid_dir))
    valid_y_data_name = glob('{}/*.*'.format(args.valid_dir[:-1] + "_Y/{}".format(args.condition)))

    valid_data_name.sort()
    valid_y_data_name.sort()

    for idx in range(len(valid_data_name)):
        valid_im = load_images(valid_data_name[idx])
        valid_data.append(valid_im)

        valid_y_im = load_images_luminance(valid_y_data_name[idx])
        valid_y_data.append(valid_y_im)

    visibility_enhance.train(train_data, train_y_data, valid_data, valid_y_data, batch_size=args.batch_size,
...
def visibility_test(visibility_enhance):
    ...
    test_y_data_name = glob(os.path.join(args.test_dir[:-1] + '_Y/{}'.format(args.condition)) + "/*.*")

    test_data_name.sort()
    test_y_data_name.sort()

    test_data = []
    test_y_data = []
    for idx in range(len(test_data_name)):
        test_im = load_images(test_data_name[idx])
        test_y_im = load_images_luminance(test_y_data_name[idx])

        test_im = np.concatenate([test_im, test_y_im], -1)
        test_data.append(test_im)
        test_y_data.append(test_y_im)

    visibility_enhance.test(test_data, test_y_data, test_data_name, args.ckpt_dir, save_dir=args.save_dir)
